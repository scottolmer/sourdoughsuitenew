...
export default function AddRecipeScreen({ navigation, route }: Props) {
  // Extract prefilled data if provided
  const prefilledFormula = route.params?.prefilledFormula;

  const [name, setName] = useState('');
  const [description, setDescription] = useState('');
  const [starterUsedId, setStarterUsedId] = useState<number | undefined>(undefined);
  const [flour, setFlour] = useState(prefilledFormula?.flour || '500');
  const [water, setWater] = useState(prefilledFormula?.water || '75');
  const [salt, setSalt] = useState(prefilledFormula?.salt || '2');
  const [starter, setStarter] = useState(prefilledFormula?.starter || '20');
  const [yieldAmount, setYieldAmount] = useState('');
  const [instructions, setInstructions] = useState('');
  const [notes, setNotes] = useState('');
  const [additionalIngredients, setAdditionalIngredients] = useState<RecipeIngredient[]>(
    prefilledFormula?.additionalIngredients?.map(ing => ({
      ...ing,
